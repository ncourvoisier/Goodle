...
public class AdminDeleteUser {
    ...
    @Et("^l'administrateur clique sur le bouton \"([^\"]*)\"$")
    public void lAdministrateurCliqueSurLeBouton(String arg0) throws Throwable {
    ...
    public void lUtilisateurSesÉvénementsOuSesVotesNeSontPasSupprimés() throws SQLException {
        // l'exigence est un peu bizarre , on va vérifier que tout à bien eté supprimé

        /*
        String sql = "SELECT * FROM Personne WHERE ID = '"+user+"';";
        PreparedStatement s = con.prepareStatement(sql);
        ResultSet personne = s.executeQuery();
        if(personne.next())
        {
            assertTrue(false);
        }
        ...
        String sql2 = "SELECT * FROM Evenement WHERE ID = '"+pastEvent2+"';";
        PreparedStatement s2 = con.prepareStatement(sql2);
        ResultSet event = s2.executeQuery();
        if(event.next())
        {
            assertTrue(false);
        }
        ...
        String sql3 = "SELECT * FROM Reponse WHERE ID = '"+reponseEvent+"';";
        PreparedStatement s3 = con.prepareStatement(sql3);
        ResultSet reponse = s3.executeQuery();
        if(reponse.next())
        {
            assertTrue(false);
        }
